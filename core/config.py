...
class Settings(BaseSettings):
    ...
    mongodb_password: Optional[SecretStr] = Field(
    ...
    max_upload_size: int = Field(
        default=52428800, le=104857600, validation_alias="MAX_UPLOAD_SIZE"
    )
    allowed_origins: list[str] = Field(
        default=["http://localhost:3000"], validation_alias="ALLOWED_ORIGINS"
    )

    model1_path: Path = Field(
        default="./NN_models/model-1.pt", validation_alias="MODEL1_PATH"
    )
    model2_path: Path = Field(
        default="./NN_models/model-2.pt", validation_alias="MODEL2_PATH"
    )
    model_timeout: int = Field(
        default=30, ge=5, le=300, validation_alias="MODEL_TIMEOUT"
    )

    output_model1_path: Path = Field(
        default="result", validation_alias="OUTPUT_MODEL1_PATH"
    )
    ...
    def mongodb_url(self) -> str:
        if self.mongodb_password is None:
            return (
                f"mongodb://{self.mongodb_user}@{self.mongodb_host}:{self.mongodb_port}"
            )
    ...
    def model_post_init(self, __context):
        for model_path in [self.model1_path, self.model2_path]:
            if not model_path.exists():
                ...
                logger.info(
                    f"✅ Model found: {model_path} ({model_path.stat().st_size} bytes)"
                )
