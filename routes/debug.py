...
async def model_status(request: Request) -> Dict[str, Any]:
    ...
    return {"models": status, "versions": versions, "model_paths": {"model1": settings.model1_path, "model2": settings.model2_path}}
...
async def test_photo(photo_path: str):
    ...
    for key, model_path in [("model1", settings.model1_path), ("model2", settings.model2_path)]:
        ...
        try:
            ...
            try:
                out = model(__import__('PIL').Image.open(p).convert('RGB'))
                t1 = time.time()
...
async def test_model1(photo_path: str):
    ...
    try:
        ...
        logger.info(f"Inference complete: output type={type(output)}, shape={output.shape if hasattr(output, 'shape') else 'N/A'}")
