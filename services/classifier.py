    ...
    try:
        for idx, img_path in enumerate(image_files):
            try:
                ...
                smoothed_conf = float(sum(confidences) / len(confidences)) if confidences else pred_conf
                ...
                if confidence < CONFIDENCE_THRESHOLD:
                    logger.warning(f"⚠️ ОТБРАКОВКА: {img_path.name} (confidence={confidence:.4f} < {CONFIDENCE_THRESHOLD})")
                    rejected_count += 1
                    continue
                # Логирование предсказания и текущего wagon_id
                logger.debug(f"📸 {img_path.name} → {class_name} (confidence={confidence:.3f}), current_wagon={current_wagon_id}")
                ...
                if class_name:
                    ...
                    logger.debug(f"   ➕ Добавлено к {current_wagon_id} ({len(current_wagon_photos)})")
                    
                    output_wagon_dir = Path.cwd() / "photo_aggregate"/ f"{timestamp}" / f"{current_wagon_id}"
                    ...
                    if current_wagon_id is not None:
                        ...
                        if count_photos < 3:
                            logger.warning(f"⚠️ Вагон #{current_wagon_id} слишком короткий ({count_photos}<3), отбрасывается")
                            # Отбраковываем
                        else:
