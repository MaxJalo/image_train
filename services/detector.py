...
def predict_model2(image: Image.Image) -> Model2Output:
    ...
    try:
        ...
        logger.info(f"✅ Model-2 result: side={side}, confidence={overall_confidence:.4f}, features={features}")
    ...
    for wagon_id, photos in wagon_groups.items():
        ...
        for photo_path, camera_id in photos:
            try:
                ...
                logger.debug(f"      Side: {model2_output.side}, confidence: {model2_output.confidence:.4f}")
        ...
        logger.info(f"✅ {wagon_id}: обработано {processed_count}/{len(photos)} фото → {final_side}")
