...
def _load_mobilenet_model(model_path: str) -> Any:
    try:
        ...
        logger.debug(f"State dict загружен: {len(result.missing_keys)} недостающих ключей, {len(result.unexpected_keys)} неожиданных ключей")
        ...
        raise ImportError("PyTorch и torchvision требуются. Установить с: pip install torch torchvision")
    except Exception as e:
...
def _load_pytorch_model(model_path: str) -> Any:
    ...
    try:
        ...
        try:
            ...
            logger.debug(f"Загрузчик YOLO не удался ({type(yolo_error).__name__}), попытка generic torch.load")
            ...
            if isinstance(loaded, dict):
                ...
                try:
                    ...
                    logger.error(f"Ошибка загрузки MobileNet: {type(mobilenet_error).__name__}: {mobilenet_error}")
                    raise Exception(f"Не удалось загрузить как state_dict MobileNet: {str(mobilenet_error)}")
