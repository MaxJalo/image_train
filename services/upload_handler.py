...
class UploadHandler:
    ...
    def _extract_metadata_from_path(
        file_path: Path, extract_root: Path
        ...
        try:
            ...
            for img_path in sorted(image_files):
                try:
                    ...
                    camera_id_from_path, train_hash, depth = (
                        UploadHandler._extract_metadata_from_path(img_path, extract_dir)
                    )
            ...
            logger.info(
                f"✅ Успешно обработано {len(extracted_files)} изображений из {len(image_files)}"
            )
            ...
            if validation_errors and len(extracted_files) < len(image_files):
                warning = (
                    f"Успешно обработано {len(extracted_files)} из {len(image_files)} файлов. "
