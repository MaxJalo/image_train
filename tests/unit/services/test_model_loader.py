...
from services.model_loader import _load_pickle_model, load_model
...
class TestModelLoader:
    def test_load_model_pt_uses_internal_loader(self, tmp_path):
        ...
        with (
            patch("services.model_loader.Path.exists", return_value=True),
            patch(
                "services.model_loader._load_pytorch_model", return_value=MagicMock()
            ) as mock_loader,
        ):
